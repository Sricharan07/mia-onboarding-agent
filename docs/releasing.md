...
The protected environment needs only a `GEMINI_API_KEY` secret. It is exposed only to the credential check and the backend process, never dependency installation, builds, or unrelated test commands. Model-independent Chrome, Edge, Firefox, and WebKit checks run before credential preflight so a provider-account problem cannot hide browser regressions. Acceptance URLs, administrator credentials, encryption/setup secrets, integration keys, database state, and evidence paths are generated inside each workflow run. Never point the release gate at a long-lived external deployment.
