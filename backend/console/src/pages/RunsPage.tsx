...
function Steps({ run }: { run: RunDetail }) {
  return run.steps.length ? <div className="run-steps">{run.steps.map((step) => { const actions = Array.isArray(step.directive.actions) ? step.directive.actions as Array<Record<string, unknown>> : []; return <article key={step.id}><header><span>{step.stepIndex}</span><div><strong>{step.progress}</strong><p>{step.assessment}</p></div><StatusBadge value={step.status} /></header><div className="step-facts"><span>Observation {step.observationRevision}</span><span>{step.model}</span><span>{formatDuration(step.latencyMs)}</span><span>{formatNumber((step.inputTokens ?? 0) + (step.outputTokens ?? 0))} tokens</span></div>{actions.length ? <div className="directive-list">{actions.map((action, index) => <div key={String(action.actionId ?? index)}><strong>{String(action.type ?? "action")}</strong><span>{String(action.message ?? "")}</span><code>{String((action.target as Record<string, unknown> | undefined)?.ref ?? action.route ?? action.hostAction ?? "")}</code></div>)}</div> : <pre className="json-inline" tabIndex={0} aria-label={`Step ${step.stepIndex} directive`}>{json(step.directive)}</pre>}{step.retrievedSources.length ? <details><summary>{step.retrievedSources.length} retrieved sources</summary><pre className="json-inline" tabIndex={0} aria-label={`Step ${step.stepIndex} retrieved sources`}>{json(step.retrievedSources)}</pre></details> : null}{step.error ? <Alert tone="danger">{step.error}</Alert> : null}</article>; })}</div> : <EmptyState title="No planner steps" detail="The agent has not planned this session yet." />;
}

function Actions({ run }: { run: RunDetail }) {
  return <div className="action-evidence"><div><h3>Confirmations</h3>{run.confirmations.length ? run.confirmations.map((confirmation) => <article key={confirmation.id}><StatusBadge value={confirmation.status} /><div><strong>{confirmation.prompt}</strong><span>{confirmation.source ? `Accepted by ${confirmation.source}` : "Not resolved"} - {formatDateTime(confirmation.createdAt)}</span></div></article>) : <p className="muted">No confirmation was required.</p>}</div><div><h3>Receipts</h3>{run.receipts.length ? run.receipts.map((receipt) => <article key={receipt.actionId}><StatusBadge value={receipt.status} /><div><strong>{receipt.type} {receipt.targetRef ? <code>{receipt.targetRef}</code> : null}</strong><span>{receipt.message}</span><details><summary>Evidence</summary><pre className="json-inline" tabIndex={0} aria-label={`${receipt.type} receipt evidence`}>{json(receipt.evidence)}</pre></details></div></article>) : <p className="muted">No action receipts were submitted.</p>}</div></div>;
}
...
function Events({ run }: { run: RunDetail }) {
  return run.events.length ? <div className="event-stream">{run.events.map((event) => <article key={event.id}><Activity /><div><strong>{event.eventType.replace(/_/g, " ")}</strong><span>{formatDateTime(event.createdAt)}</span>{Object.keys(event.payload).length ? <pre className="json-inline" tabIndex={0} aria-label={`${event.eventType.replace(/_/g, " ")} event payload`}>{json(event.payload)}</pre> : null}</div></article>)}</div> : <EmptyState title="No runtime events" detail="The SDK has not emitted diagnostics for this run." />;
}
