        ...
        <div className="split-workspace actions-workspace">
          ...
          <section className="detail-pane">
            {selected ? <><header className="detail-header"><div><div className="title-with-status"><h2>{selected.name}</h2><StatusBadge value={selected.status} /></div><p>Detected {formatDateTime(selected.firstSeenAt)} - last seen {formatDateTime(selected.lastSeenAt)}</p></div><Button size="sm" variant="secondary" onClick={() => void load()}><RefreshCw /> Refresh</Button></header>
              ...
              <div className="policy-review"><Field label="Effective risk" hint={`The SDK proposes ${selected.proposedRisk.replace(/_/g, " ")}. Policy can only stay the same or become more restrictive.`}><select value={risk} onChange={(event) => setRisk(event.target.value as RiskLevel)}>{RISKS.filter((value) => RISK_RANK[value] >= RISK_RANK[selected.proposedRisk]).map((value) => <option key={value} value={value}>{value.replace(/_/g, " ")}</option>)}</select></Field><div className="effective-policy"><span>Declared effect</span><strong>{selected.effect.replace(/_/g, " ")}</strong><small>Typed and manifest-bound</small></div><div className="effective-policy"><span>Review state</span><strong>{selected.reviewedAt ? `Reviewed ${formatDateTime(selected.reviewedAt)}` : "Not reviewed"}</strong><small>Manifest {selected.manifestHash.slice(0, 12)}</small></div></div>
              <div className="schema-view"><header><h3>Input schema</h3><span>Validated by the backend before execution</span></header><pre tabIndex={0} aria-label="Host action input schema">{json(selected.inputSchema)}</pre></div>
