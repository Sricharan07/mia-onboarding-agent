...
import type { AdminUser, CreatedIntegrationKey, GeminiStatus, IntegrationKey, Product, ScanAuth, TranscriptMode } from "../types";
...
function ProductSettings({ product, busy, save }: { product: Product; busy: boolean; save: (input: Partial<Product>) => Promise<void> }) {
  ...
  const [voice, setVoice] = useState(product.voiceConfig.voice || "Aoede");
  ...
  return <Section title="Product identity" description="Mia is bound to one production origin."><form className="settings-form" onSubmit={(event) => { event.preventDefault(); void save({ name, origin: origin.replace(/\/$/, ""), voiceConfig: { enabled: voiceEnabled, voice, language: "en-US" } }); }}><div className="form-grid two"><Field label="Product name"><input required value={name} onChange={(event) => setName(event.target.value)} /></Field><Field label="Production origin" hint="Changing this updates active integration keys and revokes existing runtime tokens."><input type="url" required value={origin} onChange={(event) => setOrigin(event.target.value)} /></Field><Field label="Voice"><input required value={voice} onChange={(event) => setVoice(event.target.value)} /></Field><div className="field"><span className="field-label">Voice availability</span><label className="switch-row"><input type="checkbox" checked={voiceEnabled} onChange={(event) => setVoiceEnabled(event.target.checked)} /><span /><strong>{voiceEnabled ? "Enabled" : "Disabled"}</strong></label></div></div><div className="form-actions"><Button type="submit" disabled={busy}><Save /> Save product</Button></div></form></Section>;
}
