...
export function decryptSecret(value: string, keyMaterial: string | undefined): string {
  ...
  try {
    ...
    decipher.setAuthTag(Buffer.from(tagEncoded, "base64url").subarray(0, TAG_BYTES));
...
export function hasSecretEncryptionKey(keyMaterial: string | undefined): boolean {
...
function deriveKey(keyMaterial: string | undefined): Buffer {
  if (!keyMaterial?.trim()) {
    throw new AppError(
      ...
      "MIA_SECRET_ENCRYPTION_KEY is required to store or read encrypted scan credentials.",
      500
    );
  }
  ...
  for (const encoding of ["base64url", "base64", "hex"] as const) {
    try {
      ...
      // Fall through to hashed material.
    }
  }
