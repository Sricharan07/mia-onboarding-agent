...
export class MossSemanticSearchAdapter implements SemanticSearchAdapter {
  ...
  async deleteByFilter(filter: Record<string, string>): Promise<void> {
    ...
    try {
      const docs = await client.getDocs(indexName) as DocumentInfo[];
  ...
  async deleteByIds(ids: string[]): Promise<void> {
    const docIds = [...new Set(ids)].filter(Boolean);
  ...
  async listIdsByFilter(filter: Record<string, string>): Promise<string[]> {
    ...
    try {
      const docs = await client.getDocs(indexName) as DocumentInfo[];
      return docs.filter((doc) => matchesFilter(doc.metadata, filter)).map((doc) => doc.id).filter(Boolean);
    } catch (error) {
      throw toMossError("MOSS_LIST_FAILED", "Moss failed to list semantic records.", error);
    }
  }
