...
This copy of the demo CRM mounts the local `../../sdk` package so the AI onboarding agent can run inside the CRM UI without modifying `example/demo-crm`.
...
Before asking the assistant to run workflows, set `NEXT_PUBLIC_MIA_API_KEY` in `.env.local` to a local console API key with `runtime:write` and `logs:write` scopes, and publish at least one workflow in the console.

Voice is enabled by default in this demo. Set `NEXT_PUBLIC_MIA_ENABLE_VOICE=false` only if you want to disable voice locally. With valid backend LiveKit, STT, and TTS config, the SDK requests browser microphone permission on load, renders the custom Mia Shadow Cursor, and publishes browser microphone audio to the LiveKit room returned by the backend. There is no Ask Mia button or mic launcher. If voice setup fails, the cursor shows the error and does not fall back to text.
